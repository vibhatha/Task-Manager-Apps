...
Job.prototype.remove = function(cb) {
  ...
  this.agenda._database.destroy(this.attrs.id, function (err, records) {
    if (err) return cb(err);
    cb(records.length);
  });

  // this.agenda._db.remove({
  //   _id: this.attrs._id
  // }, function(err, count) {
  //   if (err) {
  //     return cb(err);
...
function parsePriority(priority) {
  ...
  if (typeof priority == 'number' || !_.isNaN(+priority))
    return +priority;
  else
